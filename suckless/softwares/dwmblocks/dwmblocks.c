...
void sighandler() {
  ...
  case SIGUSR1:
    ...
    return;
  default:
    // Check if this signal has button info (ssi_int > 0)
    if (si.ssi_int > 0) {
      fprintf(stderr, "Button click detected in SIGRTMIN signal!\n");
      // Create encoded value: (block_signal << 8) | button
      int block_signal = signal - SIGRTMIN;
      int encoded = (block_signal << 8) | si.ssi_int;
      buttonhandler(encoded);
      return;
    } else {
      // Regular signal without button info
